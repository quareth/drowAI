...
_SUPPORTED_PRESET_SCHEMA_VERSION = 1
...
def _load_connection_presets_manifest(path: Path) -> tuple[ProvingConnectionPreset, ...]:
...
def _connection_preset_from_payload(payload: Any) -> ProvingConnectionPreset:
    ...
    elif endpoint_config_field != "base_url" or fixed_base_url is not None:
...
_CONNECTION_PRESETS: Mapping[str, ProvingConnectionPreset] = MappingProxyType(
    {
        preset.id: preset
        for preset in _load_connection_presets_manifest(
...
def _normalize_provider(provider: str) -> str:
    ...
    if normalized not in _FIXED_PROVIDER_ORIGINS and normalized not in _CONNECTION_PRESETS:
