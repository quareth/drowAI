"""Characterize current runtime client construction before builder extraction.

Scope: lock factory arguments, guarded callback adaptation, reasoning validation
timing, secret failure timing, and wrapper application at the existing resolver
boundary. These tests must not anticipate future builder module internals.
"""
...
def test_v2_reasoning_validation_uses_effective_profile_after_target_resolution(
    ...
    monkeypatch.setattr(resolver_module.LLMClientFactory, "get_client", fail_factory)
...
def test_guarded_executor_uses_registered_operation_and_sanitized_failure(
    ...
    monkeypatch.setattr(resolver, "resolve_target", lambda *_args, **_kwargs: target)
    monkeypatch.setattr(resolver_module, "GuardedTransport", _GuardedTransport)
    ...
    monkeypatch.setattr(resolver_module, "GuardedTransport", _FailingGuardedTransport)
    monkeypatch.setattr(resolver_module.LLMClientFactory, "get_client", fake_get_client)
