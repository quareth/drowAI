...
This document describes the **full API workflow for a single user query** in **interactive** mode only (no automatic execution). It is intended as a reference for Codex and developers: each step, endpoint, and payload is listed so the flow can be reproduced or debugged.
