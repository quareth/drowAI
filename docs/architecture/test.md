...
| PR browser core | `npm run test:e2e:pr` | Six `@pr-core` Chromium cases through the real app stack with deterministic graph behavior, HITL lifecycle coverage, and hosted-provider catalog/settings parity | Required `.github/workflows/e2e-smoke.yml` check on every pull request |
