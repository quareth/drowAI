...
- **Streaming replay:** normalized stream packets and reasoning/system logs.
  The Scout recon-agent pilot reuses this recent task replay for cards and
  drawer activity by client-side `agent_run_id` filtering; there is no durable
  indexed agent-run history API, and process-local status is used only to mark
  replayed nonterminal runs interrupted after backend restart/loss.
